...
const HOOK_VERSION_TAG: &str = concat!("# zjbar v", env!("CARGO_PKG_VERSION"));

/// Generate hook script content with version tag inserted after the shebang.
fn hook_script_content() -> String {
    let original = include_str!("../scripts/zjbar-hook.sh");
...
HOOK_PATH="$HOME/.config/zellij/plugins/zjbar-hook.sh"
...
# Remove ALL existing zjbar hook entries (any path ending in zjbar-hook.sh)
tmp=$(mktemp)
jq '
  if .hooks and (.hooks | type == "object") then
    .hooks |= with_entries(
      .value |= [
        .[] | . as $group |
        ($group.hooks // []) | map(select((.command // "") | endswith("zjbar-hook.sh") | not)) |
